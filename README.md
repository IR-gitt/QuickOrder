...
**Эффективность специалиста** увеличена на **40%** благодаря внедрению автоматизиции и формирования документов.
...
2. **API** :
    - Получение данных от Users и отправки данных на клиент.

3. **Обработка запросов**:
   - Авторизация с использованием **Spring Security** и **BCryptPasswordEncoder**.
   - Выполнение расчетов
   - Получение информации со сторонних сервисов
...
## Клиентская часть:
1. **Авторизация**:
    - Логика для отправки данных пользователя на сервер и проверки учетных данных с помощью через **RestTemplate**.
    
2. **Работа с файлами**:
    - Механизм получения файлов с сервера и сохранения их на локальном устройстве пользователя.

3. **Пользовательский интерфейс**:
   - Использование **JavaFX** для создания интерфейса десктоп приложения для вывода и отображения данных, встроенная валидация полей с визуализацией ошибок (например, строки с некорректными данными подсвечиваются красным).
   
4. **Отправка данных для расчета**:
    - Передача введенных данных из **TableView** на сервер для выполнения расчетов.
